...
// === SQL SCRIPT UPDATE (Termasuk Table Logs & Usage Tracking) ===
const REQUIRED_SQL = `
...
BEGIN
  ...
  INSERT INTO public.notifications (user_id, title, message, type) VALUES (target_user_id, '❌ Pembayaran Ditolak', 'Bukti pembayaran tidak valid atau bermasalah.', 'payment');
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 4. Enable RLS for Logs (Admin Only)
ALTER TABLE public.system_logs ENABLE ROW LEVEL SECURITY;
CREATE POLICY "Admin All Access" ON public.system_logs FOR ALL USING (true);

-- 5. Table API Key Usage (Tracking)
...
-- 6. Increment Usage Function (Fix Error [GroqService])
...
const MESSAGE_TEMPLATES = [
  { title: "🔧 Maintenance", msg: "Sistem akan maintenance pukul 23:00 WIB." },
  { title: "🎁 Bonus", msg: "Selamat! Anda dapat bonus kredit gratis." },
  { title: "⚠️ Gangguan", msg: "Server AI sedang sibuk, mohon tunggu." }
];
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ onExit }) => {
  ...
  // State History (Approved/Rejected)
  ...
  const fetchHistory = async () => {
    ...
    // Ambil yang TIDAK PENDING (Approved/Rejected)
    const { data: raw } = await supabase.from('topup_requests')
      .select('*')
      .neq('status', 'pending')
      .order('created_at', { ascending: false })
      .limit(50); // Limit 50 terakhir agar tidak berat
    ...
    <div className="min-h-screen bg-[#0f172a] text-slate-200 p-3 md:p-6 font-sans relative">
          ...
          <div className="w-full max-w-sm bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl p-6 transform scale-100 animate-in zoom-in-95">
            ...
            <div className="bg-slate-800/50 rounded-xl p-4 mb-6 border border-slate-700/50 space-y-2">
              {confirmModal.type === 'delete_history' ? (
                 <div className="text-center py-2">
                    ...
                    <p className="text-slate-300">Data riwayat pembayaran.</p>
                 </div>
              ) : (
                <>
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-bold text-white">
                      {confirmModal.req?.currency === 'IDR' ? 'Rp ' : '$'}
                      {confirmModal.req?.price.toLocaleString()}
                    </span>
                  </div>
                </>
              )}
            ...
            <div className="flex gap-3">
              <button 
                ...
                className="flex-1 py-2.5 rounded-lg border border-slate-600 text-slate-300 font-bold text-sm hover:bg-slate-800 transition-colors"
              >
                Batal
              </button>
              <button 
                ...
                disabled={confirmModal.loading}
                className={`flex-1 py-2.5 rounded-lg font-bold text-sm text-white shadow-lg flex items-center justify-center gap-2 ${confirmModal.type === 'approve' ? 'bg-green-600 hover:bg-green-500' : 'bg-red-600 hover:bg-red-500'}`}
              >
                {confirmModal.loading ? <Loader2 size={16} className="animate-spin"/> : 'Ya, Konfirmasi'}
              </button>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 border-b border-slate-800 pb-4">
          ...
          <button onClick={onExit} className="self-start md:self-auto px-4 py-2 bg-slate-800 hover:bg-slate-700 rounded-lg text-sm flex items-center gap-2 transition-colors">
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2 -mx-3 px-3 md:mx-0 md:px-0 custom-scrollbar">
           {[
             ...
             <button key={tab.id} onClick={() => setActiveTab(tab.id as any)} className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-bold whitespace-nowrap transition-all ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-lg' : 'bg-slate-800 text-slate-400'}`}>
               {tab.icon} {tab.label}
               {tab.id === 'payments' && requests.length > 0 && (
                 <span className="bg-red-500 text-white text-[10px] px-1.5 rounded-full">{requests.length}</span>
               )}
             </button>
           ))}
        </div>
        ...
        <div className="bg-slate-900/50 border border-slate-800 rounded-2xl p-4 md:p-6 min-h-[500px] shadow-2xl relative">
            ...
            <div className="animate-in fade-in">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
                <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700">
                   <p className="text-[10px] text-slate-400 uppercase font-bold">Total Keys</p>
                   <p className="text-xl font-black text-white">{keys.filter(k=>k).length}</p>
                </div>
                <div className="bg-slate-800/60 p-3 rounded-xl border border-slate-700">
                ...
                <div className="col-span-2 md:col-span-1 bg-slate-800/60 p-3 rounded-xl border border-slate-700 flex justify-between items-center">
                   <div>
                     <p className="text-[10px] text-slate-400 uppercase font-bold">Auto Refresh</p>
                     <p className="text-xs text-white">{isAutoRefresh ? 'ON (30s)' : 'OFF'}</p>
                   </div>
                   <button onClick={()=>setIsAutoRefresh(!isAutoRefresh)} className={`p-2 rounded-lg ${isAutoRefresh?'bg-green-500/20 text-green-400':'bg-slate-700 text-slate-400'}`}><RefreshCw size={16} className={isAutoRefresh?'animate-spin':''}/></button>
                </div>
              </div>
              ...
              <div className="space-y-3 max-h-[400px] overflow-y-auto pr-1">
                {keys.map((k, i) => {
                   ...
                   return (
                     <div key={i} className={`p-3 rounded-lg border flex flex-col sm:flex-row gap-3 items-start sm:items-center ${h.status==='active'?'border-green-500/30 bg-green-900/10':'border-slate-700 bg-slate-800/30'}`}>
                        <div className="flex items-center gap-2 w-full sm:w-auto">
                          ...
                          <span className="ml-auto sm:hidden text-[10px] font-mono text-slate-500">{h.latency}ms</span>
                        </div>
                        <input 
                          type={visibleKeys[i] ? "text" : "password"}
                          value={k} onChange={e=>{const n=[...keys];n[i]=e.target.value;setKeys(n)}}
                          className="flex-1 w-full bg-black/20 border border-slate-700 rounded px-3 py-2 text-xs font-mono text-slate-300 focus:border-blue-500 outline-none"
                          placeholder="API Key..."
                        />
              ...
              <div className="mt-6 pt-4 border-t border-slate-800">
                <label className="text-xs font-bold text-slate-400 mb-2 block flex items-center gap-2"><Globe size={12}/> Serper Web Search Key</label>
                <input type="password" value={serperKey} onChange={e=>setSerperKey(e.target.value)} className="w-full bg-black/20 border border-slate-700 rounded px-3 py-2 text-xs font-mono text-slate-300 outline-none focus:border-blue-500" placeholder="Serper API Key..."/>
              </div>
            </div>
          )}
          ...
          {activeTab === 'payments' && (
            <div className="animate-in fade-in">
              <div className="flex justify-between items-center mb-4">
                 ...
                 <button onClick={fetchRequests} className="p-2 bg-slate-800 rounded-lg hover:bg-slate-700"><RefreshCw size={16}/></button>
              </div>
              
              {loadingRequests ? <div className="text-center py-10"><Loader2 className="animate-spin mx-auto"/></div> : (
                <div className="grid gap-4">
                  {requests.map(req => (
                    <div key={req.id} className="bg-slate-800/50 p-4 rounded-xl border border-slate-700 flex flex-col sm:flex-row gap-4">
                       <div className="w-full sm:w-24 h-24 bg-black rounded-lg overflow-hidden shrink-0 border border-slate-600">
                         {req.proof_url ? (
                           <a href={req.proof_url} target="_blank" className="block w-full h-full hover:scale-105 transition-transform"><img src={req.proof_url} className="w-full h-full object-cover"/></a>
                         ) : <div className="flex items-center justify-center h-full"><AlertCircle size={20} className="text-slate-600"/></div>}
                       </div>
                       
                       <div className="flex-1">
                          <div className="flex justify-between items-start">
                             <div>
                               <div className="font-bold text-white">{req.profiles?.full_name || 'Unknown'}</div>
                               <div className="text-xs text-slate-400">{req.profiles?.email}</div>
                             </div>
                             <span className="text-[10px] px-2 py-0.5 rounded uppercase font-bold text-yellow-400 bg-yellow-900/20">{req.status}</span>
                          </div>
                          <div className="mt-2 bg-black/20 p-2 rounded border border-slate-700/50 flex justify-between items-center">
                             <span className="text-xs text-slate-400">Topup: <b>{req.amount} Credits</b></span>
                             <span className="text-sm font-bold text-green-400">Rp {req.price.toLocaleString()}</span>
                          </div>
                          
                          <div className="flex gap-2 mt-3">
                            <button 
                              onClick={() => openConfirmModal('approve', req)} 
                              className="flex-1 bg-green-600 hover:bg-green-500 py-2 rounded-lg text-xs font-bold text-white shadow-lg flex justify-center items-center gap-2"
                            >
                              Approve
                            </button>
                            <button 
                              onClick={() => openConfirmModal('reject', req)} 
                              className="flex-1 bg-slate-700 hover:bg-red-900/50 py-2 rounded-lg text-xs font-bold text-slate-300 hover:text-red-300 flex justify-center items-center gap-2"
                            >
                              Reject
                            </button>
                          </div>
                  ...
                  {requests.length === 0 && <p className="text-center text-slate-500 py-10">Tidak ada request pending.</p>}
                </div>
              )}
            </div>
          )}

          {/* === TAB: HISTORY (APPROVED/REJECTED) === */}
          {activeTab === 'history' && (
            <div className="animate-in fade-in flex flex-col h-[500px]">
              <div className="flex justify-between items-center mb-4">
                 <div className="flex gap-2 items-center">
                   <h2 className="font-bold text-white mr-4">History</h2>
                   {selectedHistoryIds.size > 0 && (
                     <button 
                       onClick={() => openConfirmModal('delete_history')}
                       className="bg-red-600 hover:bg-red-500 text-white px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-2 shadow-lg animate-in zoom-in"
                     >
                       <Trash2 size={14} /> Hapus ({selectedHistoryIds.size})
                     </button>
                   )}
                 </div>
                 <button onClick={fetchHistory} className="p-2 bg-slate-800 rounded-lg hover:bg-slate-700"><RefreshCw size={16} className={loadingHistory?"animate-spin":""}/></button>
              </div>

              <div className="flex-1 bg-black/40 rounded-xl border border-slate-800 overflow-hidden flex flex-col">
                {/* Table Header */}
                <div className="flex items-center px-4 py-3 bg-slate-900/80 border-b border-slate-800 text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                   <button onClick={toggleSelectAllHistory} className="mr-3 text-slate-400 hover:text-white">
                      {selectedHistoryIds.size > 0 && selectedHistoryIds.size === historyRequests.length ? <CheckSquare size={16}/> : <Square size={16}/>}
                   </button>
                   <span className="w-24">Date</span>
                   <span className="flex-1">User</span>
                   <span className="w-24 text-right pr-4">Amount</span>
                   <span className="w-20 text-center">Status</span>
                   <span className="w-12 text-center">Proof</span>
                </div>
                
                {/* Table Body */}
                <div className="overflow-y-auto flex-1 custom-scrollbar">
                   {historyRequests.length === 0 ? (
                     <div className="text-center py-20 text-slate-600 text-sm">Belum ada history.</div>
                   ) : (
                     historyRequests.map(req => (
                       <div key={req.id} className={`flex items-center px-4 py-3 border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors ${selectedHistoryIds.has(req.id) ? 'bg-blue-900/10' : ''}`}>
                          <button onClick={() => toggleSelectHistory(req.id)} className={`mr-3 ${selectedHistoryIds.has(req.id) ? 'text-blue-400' : 'text-slate-600 hover:text-slate-400'}`}>
                            {selectedHistoryIds.has(req.id) ? <CheckSquare size={16}/> : <Square size={16}/>}
                          </button>
                          
                          <span className="w-24 text-[10px] text-slate-500 font-mono">{new Date(req.created_at).toLocaleDateString()}</span>
                          
                          <div className="flex-1 min-w-0 pr-2">
                             <div className="text-xs font-bold text-white truncate">{req.profiles?.full_name || 'Unknown'}</div>
                             <div className="text-[10px] text-slate-500 truncate">{req.profiles?.email}</div>
                          </div>
                          
                          <div className="w-24 text-right pr-4">
                             <div className="text-xs font-bold text-white">{req.amount} Cr</div>
                             <div className="text-[10px] text-slate-500">Rp {req.price.toLocaleString()}</div>
                          </div>
                          
                          <div className="w-20 text-center">
                             <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold uppercase ${req.status==='approved'?'text-green-400 bg-green-900/20':'text-red-400 bg-red-900/20'}`}>
                               {req.status}
                             </span>
                          </div>
                          ...
                          <div className="w-12 flex justify-center">
                             {req.proof_url ? (
                               <a href={req.proof_url} target="_blank" className="text-blue-400 hover:text-blue-300"><FileJson size={14}/></a>
            ...
            <div className="animate-in fade-in flex flex-col h-[500px]">
              <div className="flex justify-between items-center mb-4">
                 <div className="flex gap-2">
                   <button onClick={() => setLogFilter('ALL')} className={`px-3 py-1 text-xs font-bold rounded-lg ${logFilter === 'ALL' ? 'bg-blue-600 text-white' : 'bg-slate-800 text-slate-400'}`}>All</button>
                   <button onClick={() => setLogFilter('ERROR')} className={`px-3 py-1 text-xs font-bold rounded-lg ${logFilter === 'ERROR' ? 'bg-red-600 text-white' : 'bg-slate-800 text-slate-400'}`}>Errors</button>
                   <button onClick={() => setLogFilter('SUCCESS')} className={`px-3 py-1 text-xs font-bold rounded-lg ${logFilter === 'SUCCESS' ? 'bg-green-600 text-white' : 'bg-slate-800 text-slate-400'}`}>Success</button>
                 </div>
                 <button onClick={fetchLogs} className="p-2 bg-slate-800 rounded-lg hover:bg-slate-700"><RefreshCw size={16} className={loadingLogs ? "animate-spin" : ""}/></button>
              </div>

              <div className="flex-1 bg-black/40 rounded-xl border border-slate-800 overflow-hidden flex flex-col">
                <div className="flex items-center px-4 py-2 bg-slate-900/80 border-b border-slate-800 text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                ...
                <div className="overflow-y-auto flex-1 custom-scrollbar">
                   {logs.map(log => (
                     <div key={log.id} className="border-b border-slate-800/50 hover:bg-white/5 transition-colors">
                        <div 
                          className="flex items-center px-4 py-3 cursor-pointer"
                          onClick={() => setExpandedLogId(expandedLogId === log.id ? null : log.id)}
                        >
                           <span className="w-20 text-[10px] text-slate-500 font-mono">{new Date(log.created_at).toLocaleTimeString()}</span>
                           <span className={`w-20 text-[10px] font-bold ${log.level === 'ERROR' ? 'text-red-400' : log.level === 'SUCCESS' ? 'text-green-400' : log.level === 'WARN' ? 'text-yellow-400' : 'text-blue-400'}`}>{log.level}</span>
                           <span className="w-32 text-xs font-bold text-slate-300 truncate pr-2">{log.action}</span>
                           <span className="flex-1 text-xs text-slate-400 truncate">{log.message}</span>
                        </div>
                        {expandedLogId === log.id && (
                          <div className="px-4 py-3 bg-[#0a0a0a] border-y border-slate-800">
                             <div className="pl-4 border-l-2 border-slate-700 ml-4">
                                <p className="text-xs text-slate-400 mb-1">Full ID: {log.id}</p>
                                {log.meta && <pre className="text-[10px] text-green-300 font-mono bg-black p-2 rounded border border-slate-800 overflow-x-auto">{JSON.stringify(log.meta, null, 2)}</pre>}
                             </div>
                          </div>
                        )}
                     </div>
            ...
            <div className="grid md:grid-cols-2 gap-6 animate-in fade-in">
               <div className="space-y-4">
                 ...
                 <div className="flex bg-slate-950 p-1 rounded-lg">
                    <button onClick={()=>setIsBroadcast(false)} className={`flex-1 py-2 text-xs rounded-md ${!isBroadcast?'bg-slate-800 text-white':'text-slate-500'}`}>Personal</button>
                    <button onClick={()=>setIsBroadcast(true)} className={`flex-1 py-2 text-xs rounded-md ${isBroadcast?'bg-blue-600 text-white':'text-slate-500'}`}>Broadcast All</button>
                 </div>
               ...
               <div className="bg-slate-800/30 p-4 rounded-xl border border-slate-800">
                  <h3 className="font-bold text-sm mb-3">Template Cepat</h3>
                  <div className="space-y-2">
                    {MESSAGE_TEMPLATES.map((t,i) => (
                      <button key={i} onClick={()=>{setMsgTitle(t.title);setMsgBody(t.msg)}} className="w-full text-left p-3 bg-slate-900 hover:bg-slate-800 rounded-lg border border-slate-800 transition-colors">
                        <div className="font-bold text-xs text-white mb-1">{t.title}</div>
                        <div className="text-[10px] text-slate-400 truncate">{t.msg}</div>
                      </button>
                    ))}
                  </div>
               </div>
          ...
          {activeTab === 'database' && (
             <div className="h-full flex flex-col">
               ...
               <p className="text-center text-xs text-slate-500 mt-4">Copy script di atas dan jalankan di Supabase SQL Editor untuk update sistem log.</p>
             </div>
          )}
